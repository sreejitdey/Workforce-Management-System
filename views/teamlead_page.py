"""
Team Lead Dashboard Module.

This module provides a Streamlit-based interface specifically tailored for Team Leads.
It allows them to:
1. Update their own daily availability status (e.g., Available, Planned Leave).
2. Update the availability status for any associate explicitly mapped to them.
3. View and bulk-edit the availability of their entire team over a selected date range using an interactive data grid.
4. Manage their own login password.
"""
...
def teamlead_dashboard(user):
    """
    Renders the main dashboard interface for a logged-in Team Lead.
    ...
    Args:
        user (User): The ORM object representing the currently authenticated Team Lead.
    """
    ...
    if menu == "Update Availability":
        ...
        # Sub-menu to toggle between updating their own calendar or a team member's calendar
        submenu = st.pills(
            "Choose Update Option", ["Self", "Team Members"], default="Self"
        )
        ...
        if submenu == "Self":
            ...
            selected_status = st.selectbox(
                "Select Status", status_options, key="selected_status"
            )
        ...
        # --- Option B: Update Team Members' Availability ---
        elif submenu == "Team Members":
            # Fetch all mapping records where this user is the designated Team Lead
            mappings = (
            ...
            # Retrieve the actual Employee records for those mapped emails
            employees = (
                session.query(Employee).filter(Employee.email.in_(emp_emails)).all()
            )

            if not employees:
                st.warning("No associates mapped to you yet")
                return

            # Create a dictionary to map employee names to their emails for the dropdown UI
            emp_dict = {e.name: e.email for e in employees}
            selected_emp = st.selectbox("Select Team Member", list(emp_dict.keys()))
            ...
            if st.button("Save Details", type="primary"):
                # Look up the selected employee's email using the dictionary
                emp_email = emp_dict[selected_emp]
                delta = end_date - start_date
                ...
                session.commit()
                st.success(
                    f"Status '{selected_status}' applied for {selected_emp} from {start_date} to {end_date}"
                )
    ...
    # ==========================================
        ...
        # Determine the scope of people to display (Team members + the Team Lead themselves)
        mappings = (
            session.query(TeamMapping)
            .filter(TeamMapping.teamlead_email == user.email)
            .all()
        ...
        emp_emails = [m.employee_email for m in mappings]
        emp_emails.append(user.email)  # Append the Team Lead to the list

        # Fetch records for everyone in scope
    ...
    # ==========================================
        ...
        if st.button("Update Password", type="primary"):
            ...
            if new_password != confirm_password:
                ...
                usr = (
                    session.query(User)
                    .filter(User.email == user.email, User.role == "teamlead")
                    .first()
                )
                if not usr:
